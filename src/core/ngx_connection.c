...
ngx_open_listening_sockets(ngx_cycle_t *cycle)
{
    ...
    { // bind和listen最多重试5次
        ...
        for (i = 0; i < cycle->listening.nelts; i++)
        {
            ...
            { // 例如热升级的时候，fd是通过NGINX环境变量继承过来的，这里fd大于0
                continue;
            }
...
#endif
            ngx_log_stderr(0, "<%s:%d %s>: ls[%d]->backlog ->%d", __FILE__, __LINE__, __FUNCTION__, i, ls[i].backlog);
    ...
    for (i = 0; i < cycle->listening.nelts; i++)
    {
        ...
        if (ls[i].rcvbuf != -1)
        {
            if (setsockopt(ls[i].fd, SOL_SOCKET, SO_RCVBUF,
                           (const void *)&ls[i].rcvbuf, sizeof(int)) == -1)
            {
                ngx_log_error(NGX_LOG_ALERT, cycle->log, ngx_socket_errno,
        ...
        if (ls[i].sndbuf != -1)
        {
            if (setsockopt(ls[i].fd, SOL_SOCKET, SO_SNDBUF,
                           (const void *)&ls[i].sndbuf, sizeof(int)) == -1)
            {
                ngx_log_error(NGX_LOG_ALERT, cycle->log, ngx_socket_errno,
        ...
        if (ls[i].keepcnt)
        {
            if (setsockopt(ls[i].fd, IPPROTO_TCP, TCP_KEEPCNT,
                           (const void *)&ls[i].keepcnt, sizeof(int)) == -1)
            {
                ngx_log_error(NGX_LOG_ALERT, cycle->log, ngx_socket_errno,
        ...
        if (ls[i].setfib != -1)
        {
            if (setsockopt(ls[i].fd, SOL_SOCKET, SO_SETFIB,
                           (const void *)&ls[i].setfib, sizeof(int)) == -1)
            {
                ngx_log_error(NGX_LOG_ALERT, cycle->log, ngx_socket_errno,
        ...
        if (ls[i].fastopen != -1)
        {
            if (setsockopt(ls[i].fd, IPPROTO_TCP, TCP_FASTOPEN,
                           (const void *)&ls[i].fastopen, sizeof(int)) == -1)
            {
                ngx_log_error(NGX_LOG_ALERT, cycle->log, ngx_socket_errno,
...
┗━━━━━━━━━━━━━━━━━━━┻━━━━━━━━━━━━━━┻━━━━━━━━━━━━━━━━━━━┛
*/
void ngx_free_connection(ngx_connection_t *c) // 归还c到连接池中
{
...
void ngx_reusable_connection(ngx_connection_t *c, ngx_uint_t reusable) // 和下面的ngx_drain_connections配合
{
...
ngx_connection_error(ngx_connection_t *c, ngx_err_t err, char *text)
{
    ...
    if (err == 0 || err == NGX_ECONNRESET
        ...
        || err == NGX_EPIPE
#endif
        || err == NGX_ENOTCONN || err == NGX_ETIMEDOUT || err == NGX_ECONNREFUSED || err == NGX_ENETDOWN || err == NGX_ENETUNREACH || err == NGX_EHOSTDOWN || err == NGX_EHOSTUNREACH)
    {
        switch (c->log_error)
        {
