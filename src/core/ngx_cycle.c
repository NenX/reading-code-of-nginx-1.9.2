...
ngx_init_cycle(ngx_cycle_t *old_cycle)
{
    ...
    */
    ...
    {
        if (i >= part->nelts)
        { // 在热启动nginx的时候才会执行下面的相关代码，默认i和part->nelts都等于0
            if (part->next == NULL)
            {
                break;
            }
    ...
    */
    ...
    for (i = 0; /* void */; i++)
    {
        ...
        for (n = 0; /* void */; n++)
        {
            ...
            if (oshm_zone[i].shm.name.len == shm_zone[n].shm.name.len && ngx_strncmp(oshm_zone[i].shm.name.data,
...
ngx_shared_memory_add(ngx_conf_t *cf, ngx_str_t *name, size_t size, void *tag)
{
    ...
    for (i = 0; /* void */; i++)
    {
        ...
        { // 例如proxy_cache abc, fastcgi abc；同时配置，就会报错
