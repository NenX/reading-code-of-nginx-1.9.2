...
{                                                              // ctx为&u->output， in为u->request_bufs这里nginx filter的主要逻辑都在这个函数里面,将in参数链表的缓冲块拷贝到
    ...
    if (in)
    { // 拷贝一份数据到ctx->in里面，需要老老实实的进行数据拷贝了。将in参数里面的数据拷贝到ctx->in里面。换了个in
    ...
    { // 循环读取缓存中或者内存中的数据发送
      ...
      // 实际上在接受完后端数据后，在想客户端发送包体部分的时候，会两次调用该函数，一次是ngx_event_pipe_write_to_downstream-> p->output_filter(),
        ...
        { // 遍历所有待发送的数据。将他们一个个拷贝到out指向的链表中
            ...
            if (ctx->buf == NULL)
            { // 每次拷贝数据前，先给ctx->buf分配空间，在下面的ngx_output_chain_get_buf函数中
...
{                                                                   // 看看这个节点是否可以拷贝。检测content是否在文件中。判断是否需要复制buf.
    ...
    { // 文件中内容超过了sendfile的最大上限,则只有重新读取文件到内存中发送
        sendfile = 0;
    }
    ...
    if (!sendfile)
    {
        ...
        { // 一般不启用sendfile on的时候从这里退出去  如果启用了aio on，没有启用sendfile on的情况下，也会从这里出去
...
// aio on | off | threads[=pool];
#if (NGX_HAVE_FILE_AIO)
        if (ctx->aio_handler)
        { // aio on的情况下  ngx_output_chain_copy_buf  ngx_file_aio_read
            n = ngx_file_aio_read(src->file, dst->pos, (size_t)size,
                                  src->file_pos, ctx->pool);
            if (n == NGX_AGAIN)
            { // 第一次到这里表示开始通知内核通过AIO异步读取数据，正常情况下回返回NGX_AGAIN
        ...
        { // aio thread=poll的情况
            ...
            if (n == NGX_AGAIN)
            { // 第一次到这里表示开始通知线程池读取数据，正常情况下回返回NGX_AGAIN
                ctx->aio = 1;
                return NGX_AGAIN;
            }
        ...
        if (sendfile)
        {                     // 如果是sendfile则通过上面的ngx_read_file会从磁盘文件读取一份到用户空间
