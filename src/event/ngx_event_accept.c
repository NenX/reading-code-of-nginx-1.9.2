...
{                                 // 一个accept事件对应一个ev，如当前一次有4个客户端accept，应该对应4个ev事件，一次来多个accept的处理在下面的do {}while中实现
    ...
    { /* 如果是一次读取一个accept事件的话，循环体只执行一次， 如果是一次性可以读取所有的accept事件，则这个循环体执行次数为accept事件数*/
        ...
        if (s == (ngx_socket_t)-1)
        {
            ...
            if (err == NGX_EAGAIN)
            { // 如果event{}开启multi_accept，则在accept完该listen ip:port对应的ip和端口连接后，会通过这里返回
            ...
            if (err == NGX_EMFILE || err == NGX_ENFILE)
            {
                ...
                { ////如果是不需要实现负载均衡，则扫尾延时下继续在ngx_process_events_and_timers中accept
        ...
        ngx_accept_disabled = ngx_cycle->connection_n / 8 - ngx_cycle->free_connection_n; // 判断可用连接的数目和总数目的八分之一大小，如果可用的小于八分之一，为正
...
#if (NGX_DEBUG)
        {
            ...
            for (i = 0; i < ecf->debug_connection.nelts; i++)
            {
                ...
                case AF_INET6:
                    ...
                    for (n = 0; n < 16; n++)
                    {
                        if ((sin6->sin6_addr.s6_addr[n] & cidr[i].u.in6.mask.s6_addr[n]) != cidr[i].u.in6.addr.s6_addr[n])
                        {
                            goto next;
                        }
                    }
                ...
                default: /* AF_INET */
                    ...
                    if ((sin->sin_addr.s_addr & cidr[i].u.in.mask) != cidr[i].u.in.addr)
                    {
                        goto next;
                    }
                    break;
