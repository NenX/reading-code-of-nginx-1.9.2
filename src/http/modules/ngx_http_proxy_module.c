...
typedef struct
{ // 赋值见ngx_http_file_cache_set_slot
...
{                                      // ngx_http_proxy_redirect中创建该结构空间
...
static ngx_int_t ngx_http_proxy_handler(ngx_http_request_t *r)
{
    ...
    {   
        if (plcf->vars.key_start.data == NULL)
        {
            ngx_log_error(NGX_LOG_ALERT, r->connection->log, 0, "vars.key_start.data=NULL");
        }
        else
        {   //DEBUG: 走到这里
            ngx_log_error(NGX_LOG_ALERT, r->connection->log, 0, "vars.key_start=%c ", plcf->vars.key_start.data);
        };
    ...
    if (!plcf->upstream.request_buffering && plcf->body_values == NULL && plcf->upstream.pass_request_body && (!r->headers_in.chunked || plcf->http_version == NGX_HTTP_VERSION_11))
    {
...
这个过程看起来似乎让人困惑。为什么有时需要把引用计数加1，有时却不需要呢？因为ngx_http_read_client_request_body读取请求包体是
...
这个问题）。在HTTP反向代理模块中，它的ngx_http_proxy_handler方法中用“ngx_http_read_client_request_body(r，ngx_http_upstream_init);”
...
//TIP: 读取client请求体再后，再调用 ngx_http_upstream_init 初始化
...
ngx_http_proxy_create_key(ngx_http_request_t *r)
{
    ...
    { // 如果设置了proxy_cache_key

        if (ngx_http_complex_value(r, &plcf->cache_key, key) != NGX_OK)
        { // 解析plcf->cache_key中的变量value值到r->cache->keys中
            return NGX_ERROR;
        }
...
ngx_http_proxy_create_request(ngx_http_request_t *r) // ngx_http_upstream_init_request中执行
{
    ...
    {
        loc_len = (r->valid_location && ctx->vars.uri.len) ? plcf->location.len : 0; // 当前location的名字 location xxx {} 中的xxx的长度3
        ...
        /*
           ...
           plcf->location:/proxy1111, ctx->vars.uri:/tttxxsss,  r->uri:/proxy1111/yangtest, r->args:, urilen:19
          */
        uri_len = ctx->vars.uri.len + r->uri.len - loc_len + escape + sizeof("?") - 1 + r->args.len; // 注意sizeof("?")是2字节
    }
    ...
    { // 是否要将HTTP请求头部的HEADER发送给后端，已HTTP_为前缀
        ...
        for (i = 0; /* void */; i++)
        {
            ...
            len += header[i].key.len + sizeof(": ") - 1 + header[i].value.len + sizeof(CRLF) - 1; // 计算ngx_http_proxy_headers proxy_set_header设置的头部行以及客户端发送过来的头部行的key:value总长度
        }
    }
...
ngx_http_proxy_process_header(ngx_http_request_t *r)
{
    ...
    for (;;)
    { // 启用cache的情况下，注意这时候buf的内容实际上已经在ngx_http_upstream_process_header中出去了为缓存如文件中预留的头部内存

        rc = ngx_http_parse_header_line(r, &r->upstream->buffer, 1);

        if (rc == NGX_OK)
        { // 下面将这个新的头部字段保存起来，保存到headers_in.headers
        ...
        { // 全部都已经解析完毕了，肯定是碰到空行\R\N了
            ...
            { // 如果后端没有发送server:xxx头部行，则直接添加server: value长度为0
                h = ngx_list_push(&r->upstream->headers_in.headers);
                if (h == NULL)
                {
                    return NGX_ERROR;
            ...
            { // chunked编码方式就不需要包含content-length: 头部行，包体长度有chunked报文格式指定包体内容长度
            ...
            if (u->headers_in.status_n == NGX_HTTP_NO_CONTENT || u->headers_in.status_n == NGX_HTTP_NOT_MODIFIED || ctx->head || (!u->headers_in.chunked && u->headers_in.content_length_n == 0))
            {
                                       ...
                                       ngx_table_elt_t *h, size_t prefix, size_t len, ngx_http_proxy_rewrite_t *pr)
{
    ...
    if (pattern.len > len || ngx_rstrncmp(h->value.data + prefix, pattern.data,
                                          pattern.len) != 0)
    {
        return NGX_DECLINED;
    }
...
ngx_http_proxy_create_main_conf(ngx_conf_t *cf)
{
    ...
    if (ngx_array_init(&conf->caches, cf->pool, 4,
                       sizeof(ngx_http_file_cache_t *)) != NGX_OK)
    {
        return NULL;
    }
#endif
...
ngx_http_proxy_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    if (ngx_conf_merge_path_value(cf, &conf->upstream.temp_path,
                                  ...
                                  &ngx_http_proxy_temp_path) != NGX_OK)
    {
        return NGX_CONF_ERROR;
    }
    ...
    if (conf->method.len && conf->method.data[conf->method.len - 1] != ' ')
    {
                            ...
                            ngx_http_proxy_headers_t *headers, ngx_keyval_t *default_headers)
{
    ...
    { // 如果不为空，返回，如果是第一次进入该函数，则走下面的流程创建空间初始化
        return NGX_OK;
    }
    ...
    { // 把proxy_set_header设置的头部信息添加到headers_merged数组中
    ...
    while (h->key.len)
    { // 把default_headers(ngx_http_proxy_headers或者ngx_http_proxy_cache_headers)中不重复的元素添加到headers_merged中
    ...
    for (i = 0; i < headers_merged.nelts; i++)
    {
        ...
        { // value为空的，不用添加到headers->lengths中
            continue;
        }
        ...
        if (ngx_http_script_variables_count(&src[i].value) == 0)
        {
            ...
            copy->len = src[i].key.len + sizeof(": ") - 1 + src[i].value.len + sizeof(CRLF) - 1; // key:value字符串长度
            // 这个长度最后在解析key:value值的时候从ngx_http_script_copy_len_code获取

            // key:value字符串code
            size = (sizeof(ngx_http_script_copy_code_t) + src[i].key.len + sizeof(": ") - 1 + src[i].value.len + sizeof(CRLF) - 1 + sizeof(uintptr_t) - 1) & ~(sizeof(uintptr_t) - 1); // 4字节对齐
            ...
            copy->len = src[i].key.len + sizeof(": ") - 1 + src[i].value.len + sizeof(CRLF) - 1;
        ...
        else
        {
            ...
            size = (sizeof(ngx_http_script_copy_code_t) + src[i].key.len + sizeof(": ") - 1 + sizeof(uintptr_t) - 1) & ~(sizeof(uintptr_t) - 1);
            ...
            *p = ' ';
            ...
            size = (sizeof(ngx_http_script_copy_code_t) + sizeof(CRLF) - 1 + sizeof(uintptr_t) - 1) & ~(sizeof(uintptr_t) - 1);
...
{ // 在NGX_HTTP_CONTENT_PHASE里面调用这个handler，即ngx_http_proxy_handler。
    ...
    url = &value[1];
    printf("proxy_url++++++++++++++ url: %s,%lu,%lu \n", url->data, sizeof(url->data), url->len);
    //
    n = ngx_http_script_variables_count(url);

    if (n)
    { // proxy_pass xxxx中是否带有变量，如果有变量，就解析对应的变量值
    ...
    plcf->url = *url;
    printf("proxy_url --------------- url: %s,%lu,%lu  %s,%lu,%lu \n", url->data, sizeof(url->data), url->len,
           plcf->url.data, sizeof(plcf->url.data), plcf->url.len
...
ngx_http_proxy_set_ssl(ngx_conf_t *cf, ngx_http_proxy_loc_conf_t *plcf)
{
    ...
    if (plcf->ssl_certificate.len)
    {
        ...
        if (ngx_ssl_certificate(cf, plcf->upstream.ssl, &plcf->ssl_certificate,
                                &plcf->ssl_certificate_key, plcf->ssl_passwords) != NGX_OK)
        {
            return NGX_ERROR;
        }
    }
    ...
    if (SSL_CTX_set_cipher_list(plcf->upstream.ssl->ctx,
                                (const char *)plcf->ssl_ciphers.data) == 0)
    {
...
{ // AF_UNIX代表域套接字
    if (u->family != AF_UNIX)
    { // ngx_http_upstream_add->ngx_parse_inet_url里面设置为AF_INET
