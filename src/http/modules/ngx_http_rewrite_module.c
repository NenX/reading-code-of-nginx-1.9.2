...
{ // 下面的解析以://比如rewrite  ^(/xyz/aa.*)$   http://$http_host/aa.mp4   break;为例
    ...
    if (ngx_strncmp(value[2].data, "http://", sizeof("http://") - 1) == 0 || ngx_strncmp(value[2].data, "https://", sizeof("https://") - 1) == 0 || ngx_strncmp(value[2].data, "$scheme", sizeof("$scheme") - 1) == 0)
    ...
    { // 如果没有变量，那就将lengths置空，这样就不用做多余的正则解析而直接进入字符串拷贝codes
        regex->lengths = NULL;
    }
...
ngx_http_rewrite_return(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (ret->status == (uintptr_t)NGX_ERROR)
    {
        ...
        if (cf->args->nelts == 2 && (ngx_strncmp(p, "http://", sizeof("http://") - 1) == 0 || ngx_strncmp(p, "https://", sizeof("https://") - 1) == 0 || ngx_strncmp(p, "$scheme", sizeof("$scheme") - 1) == 0))
        {
...
ngx_http_rewrite_if_condition(ngx_conf_t *cf, ngx_http_rewrite_loc_conf_t *lcf)
{
    ...
    if (len > 1 && p[0] == '$')
    {
        ...
        if ((len == 1 && p[0] == '~') || (len == 2 && p[0] == '~' && p[1] == '*') || (len == 2 && p[0] == '!' && p[1] == '~') || (len == 3 && p[0] == '!' && p[1] == '~' && p[2] == '*'))
        {
            regex = ngx_http_script_start_code(cf->pool, &lcf->codes,
                                               sizeof(ngx_http_script_regex_code_t));
            if (regex == NULL)
