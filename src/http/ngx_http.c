...
{ // 图形化参考:深入理解NGINX中的图9-2  图10-1  图4-2，结合图看,并可以配合http://tech.uc.cn/?p=300看
    ...
    */
    for (m = 0; ngx_modules[m]; m++)
    { // 注意这里为所有的NGX_HTTP_MODULE开辟了main_conf srv_conf loc_conf空间，也就是在http{}的时候为所有main srv loc开辟了空间
        if (ngx_modules[m]->type != NGX_HTTP_MODULE)
        { // http{}相关配置结构创建首先需要执行ngx_http_core_module，而后才能执行对应的http子模块
            continue;
        }
    ...
    */
    ...
    // ???: ngx_http_init_phases，在 postconfiguration 执行之前初始化
    ...
    *cf = pcf; // 恢复到上层的ngx_conf_s地址
    ...
    /// ???: ngx_http_init_phase_handlers，在 postconfiguration 执行之后初始化
    ...
    // TIP: 调用ngx_http_init_listening 调用 ngx_http_add_listening，后者设置 ls->handler = ngx_http_init_connection
...
ngx_http_init_phase_handlers(ngx_conf_t *cf, ngx_http_core_main_conf_t *cmcf)
{
    ...
    for (i = 0; i < NGX_HTTP_LOG_PHASE; i++)
    {
        ...
        case NGX_HTTP_POST_ACCESS_PHASE: // 该阶段则不允许HTTP模块加入自己的ngx_http_handler_pt方法处理用户请求方法，直接把http框架方法加入到cmcf->phase_engine.handlers数组中
            if (use_access)
            { // 只有配置了try_files aaa bbb后才会在 cmcf->phase_engine.handlers添加节点pt，见ngx_http_init_phase_handlers，如果没有配置，则直接跳过try_files阶段
                       ...
                       ngx_http_module_t *module, ngx_uint_t ctx_index)
{
    ...
    */
    ...
    for (s = 0; s < cmcf->servers.nelts; s++)
    {
        ...
        { // 以server为例，saved.srv_conf[ctx_index]为http{}中的srv_conf，cscfp[s]->ctx->srv_conf[ctx_index]为server{}中的srv_conf
                        ...
                        ngx_http_core_loc_conf_t *pclcf) // 形成3叉树见ngx_http_init_static_location_trees
{
         ...
         q = ngx_queue_next(q)) // 找到queue中正则表达式和命名location中的位置
    {
        ...
        { // 这里是一个递归，如果存在location下面还有locations的话还会进行递归调用
            return NGX_ERROR;
        }
       ...
       */
        ...
        if (clcf->noname)
        { // 只要遇到name，则直接返回，在后面拆分。注意未命名的在队列尾部，所以不会影响到前面的named和regx
            break;
        }
    }
    ...
    */
    ...
    {                                         // 如果队列里面有noname，则以该noname头拆分为locations和tail两个队列链表
...
// cscf为该LISTEN IP:PORT所处server{}上下文
static ngx_int_t
ngx_http_add_addresses(ngx_conf_t *cf, ngx_http_core_srv_conf_t *cscf, ngx_http_conf_port_t *port, ngx_http_listen_opt_t *lsopt)
{
    ...
    for (i = 0; i < port->addrs.nelts; i++)
    {
        ...
        if (lsopt->default_server)
        {
            ...
            if (default_server)
            { // 如果前面相同的ip:port所处server{}中已经配置过default_server，则报错
                ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
...
ngx_http_add_server(ngx_conf_t *cf, ngx_http_core_srv_conf_t *cscf, ngx_http_conf_addr_t *addr)
{
    ...
    else
    {
        ...
        {
            if (server[i] == cscf)
            { // 如果在同一个serve{}中配置两条相同的listen ip:port，两次的ip和port完全相等，则会走到这里
                ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
...
ngx_http_init_listening(ngx_conf_t *cf, ngx_http_conf_port_t *port)
{
    ...
    while (i < last)
    {
        ...
        {        // 如果是通配符*:port,或者是listen配置没有加bind参数
        ...
        default: /* AF_INET */
            ...
            { // 后面有addr++，所以这里的addr对应的是addr[i]的地址
                return NGX_ERROR;
            }
            break;
        }
...
ngx_http_add_listening(ngx_conf_t *cf, ngx_http_conf_addr_t *addr)
{
    ...
    // TIP: 设置 ngx_http_init_connection
                   ...
                   ngx_http_conf_addr_t *addr)
{
    ...
    for (i = 0; i < hport->naddrs; i++)
    {
        ...
        if (addr[i].hash.buckets == NULL && (addr[i].wc_head == NULL || addr[i].wc_head->hash.buckets == NULL) && (addr[i].wc_tail == NULL || addr[i].wc_tail->hash.buckets == NULL)
#if (NGX_PCRE)
            && addr[i].nregex == 0
#endif
        )
