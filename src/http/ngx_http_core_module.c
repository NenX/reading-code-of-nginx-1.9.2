...
void ngx_http_handler(ngx_http_request_t *r) /* 执行11个阶段的指定阶段 */
{
    ...
    r->gzip_vary = 0;
#endif
    // TIP: 在上一步 ngx_http_process_request 设置 ngx_http_request_handler 会调用 r->read_event_handler 或者 r->write_event_handler
...
// ngx_http_process_request->ngx_http_handler->ngx_http_core_run_phases
// TIP: ngx_http_process_request 设置事件的 handler 为 ngx_http_request_handler, ngx_http_request_handler 中调用 r->write_event_handler 所指向 ngx_http_core_run_phases
void ngx_http_core_run_phases(ngx_http_request_t *r) // 执行该请求对于的阶段的checker(),并获取返回值
{
    ngx_int_t rc;
    ...
    { // 处于同一ngx_http_phases阶段的所有ngx_http_phase_handler_t的checker指向相同的函数，见ngx_http_init_phase_handlers
        ...
        */
        ...
        if (rc == NGX_OK)
        { // 执行phase_handler阶段的hecker  handler方法后，返回值为NGX_OK，则直接退出，否则继续循环执行checker handler
            return;
        }
    }
}
...
ngx_http_core_generic_phase(ngx_http_request_t *r, ngx_http_phase_handler_t *ph)
{
    ...
    if (rc == NGX_OK)
    {                                // 如果handler方法返回NGX OK，之后将进入下一个阶段处理，而不佥理会当前阶段中是否还有其他的处理方法
    ...
    */
    ...
    { // phase_handler没有发生变化，当这个请求上的事件再次触发的时候继续在该阶段执行
        return NGX_OK;
    }
...
ngx_http_core_rewrite_phase(ngx_http_request_t *r, ngx_http_phase_handler_t *ph)
{
     ...
     */
    if (rc == NGX_DONE)
    { // phase_handler没有发生变化，因此如果该请求的事件再次触发，还会接着上次的handler执行
        return NGX_OK;
    }
                                ...
                                ngx_http_phase_handler_t *ph)
{
    ...
    if (r->headers_in.content_length_n != -1 && !r->discard_body && clcf->client_max_body_size && clcf->client_max_body_size < r->headers_in.content_length_n) // 长度超长了。拒绝
    {
    ...
    { // auto redirect,需要进行重定向。下面就给客户端返回301，带上正确的location头部
        ...
        if (r->args.len == 0)
        { // 如果客户端请求没用带参数，比如请求是: GET /AAA/BBB/
            r->headers_out.location->value = clcf->name;
        }
        else
        { // 如果客户端请求有带参数，比如请求是: GET /AAA/BBB/?query=word，则需要将参数也带在location后面
...
先把location块对应的ngx_http_core_loc_conf t配置结构体取出来，因为这里有一个配置项satisfy是下一步需要用到的。
    ...
    */
    ...
    { // 必须NGX—HTTP—ACCESS—PHASE阶段的所有handler都返回NGX_OK才算具有权限访问
    ...
    else
    {
        ...
        { // 虽然当前模块的handler任务没权限，单后面其他模块的handler可能允许该客户端访问
                              ...
                              ngx_http_phase_handler_t *ph) // 如果try_files中的文件存在，则修改uri为该文件名
{
    ...
    for (;;)
    {
        ...
        if (tf->lengths)
        { // 如果try_files后面跟的参数带有变量等，则需要把变量解析为对应的字符串，初始赋值见ngx_http_core_try_files
                            ...
                            ngx_http_phase_handler_t *ph)
{
    ...
    { // 如果在clcf->handler中设置了方法，则直接从这里进去执行该方法，然后返回，就不会执行content阶段的其他任何方法了，参考例子ngx_http_mytest_handler
...
void ngx_http_update_location_config(ngx_http_request_t *r)
{
    ...
    { // 配置client_body_in_file_only on | clean
    ...
    { // 配置client_body_in_file_only off默认是该配置
...
ngx_http_core_find_location(ngx_http_request_t *r) // 图解参考http://blog.chinaunix.net/uid-27767798-id-3759557.html
{
    ...
    ngx_int_t n;
    ngx_uint_t noregex;
    ngx_http_core_loc_conf_t *test, *clcf, **clcfp;
    ...
    if (rc == NGX_AGAIN)
    { // 这里代表的是非exact精确匹配成功的。肯定到这还不是正则成功的。
    ...
    {              // 返回这两个值表示找到对应的location{},不需要再进行查找正则表达式
   ...
   */
    ...
    { // 用了正则表达式，而且呢，regex_locations正则表达式列表里面有货，那就匹配之。

        for (clcfp = pclcf->regex_locations; *clcfp; clcfp++)
        { // 对每一个正则表达式，匹配之。

            ngx_log_debug1(NGX_LOG_DEBUG_HTTP & 1, r->connection->log, 0,
                           "ngx pcre test location: ~ \"%V\"", &(*clcfp)->name);
            test = (*clcfp);
            printf("ngx pcre test location: ~ %s\n", (char *)test->name.data);
...
ngx_http_set_content_type(ngx_http_request_t *r)
{
    ...
    if (r->exten.len)
    {
        ...
        if (type)
        { // 遍历types_hash表，然后把对应的value存在于types_hashcontent_type
                       ...
                       ngx_str_t *ct, ngx_http_complex_value_t *cv)
{
    ...
    if (status == NGX_HTTP_MOVED_PERMANENTLY || status == NGX_HTTP_MOVED_TEMPORARILY || status == NGX_HTTP_SEE_OTHER || status == NGX_HTTP_TEMPORARY_REDIRECT)
    {
        ngx_http_clear_location(r);
...
ngx_http_auth_basic_user(ngx_http_request_t *r)
{
    ...
    if (encoded.len < sizeof("Basic ") - 1 || ngx_strncasecmp(encoded.data, (u_char *)"Basic ",
                                                              sizeof("Basic ") - 1) != 0)
    {
        r->headers_in.user.data = (u_char *)"";
        return NGX_DECLINED;
...
ngx_http_gzip_ok(ngx_http_request_t *r)
{
    ...
    if (cc->elts)
    {
        ...
        if ((p & NGX_HTTP_GZIP_PROXIED_NO_CACHE) && ngx_http_parse_multi_header_lines(cc, &ngx_http_gzip_no_cache,
                                                                                      NULL) >= 0)
        {
            goto ok;
        }

        if ((p & NGX_HTTP_GZIP_PROXIED_NO_STORE) && ngx_http_parse_multi_header_lines(cc, &ngx_http_gzip_no_store,
                                                                                      NULL) >= 0)
        {
            goto ok;
        }

        if ((p & NGX_HTTP_GZIP_PROXIED_PRIVATE) && ngx_http_parse_multi_header_lines(cc, &ngx_http_gzip_private,
                                                                                     NULL) >= 0)
        {
            goto ok;
        }
                           ...
                           ngx_str_t *uri, ngx_str_t *args)
{
    ...
    if (r->uri_changes == 0)
    { // 初始设置为NGX_HTTP_MAX_URI_CHANGES，即最多重定向10此，这是为了避免循环重定向的问题。
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "rewrite or internal redirection cycle "
...
{ // 图形化参考:深入理解NGINX中的图9-2  图10-1  图4-2，结合图看,并可以配合http://tech.uc.cn/?p=300看
    ...
    *cf = pcf; // 恢复到之前server{}中的
    ...
    { // 如果没有通过"listen portnum"设置该server监听的端口，则使用下面默认的端口
...
ngx_http_core_create_main_conf(ngx_conf_t *cf)
{
    ...
    if (ngx_array_init(&cmcf->servers, cf->pool, 4,
                       sizeof(ngx_http_core_srv_conf_t *)) != NGX_OK)
    {
        return NULL;
    }
...
ngx_http_core_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    if (ngx_conf_merge_path_value(cf, &conf->client_body_temp_path,
                                  ...
                                  &ngx_http_client_temp_path) != NGX_OK)
    {
        return NGX_CONF_ERROR;
    }
    ...
    if (conf->gzip_disable_degradation == 3)
    {
        conf->gzip_disable_degradation =
            (prev->gzip_disable_degradation == 3) ? 0 : prev->gzip_disable_degradation;
    }
...
ngx_http_core_listen(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    ngx_log_stderr(0, "<%s:%d %s>: listen --> %s,  wildcard:%d",
                   __FILE__, __LINE__, __FUNCTION__,
                   value[1].data,
                   u.wildcard);
    ngx_memzero(&lsopt, sizeof(ngx_http_listen_opt_t));
    // TIP: u.sockaddr 为一个数组，前0-1字节为 family(一般为2) 的值，2-3字节为 port，4-7为 addr
    // TIP: lsopt.u 为一个 uion, 这里 lsopt.u.sockaddr.sa_family 为两字节的 family，lsopt.u.sockaddr.sa_data 为数组，包含为两字节 port 和四字节的 addr
    ...
    lsopt.fastopen = -1;
#endif
    // TIP: u.wildcard 的值：(listen 127.0.0.1:5141) 为 0，(listen *:5142)和(listen 5143)为 1
    // TIP: u.naddrs 的值：(listen 127.0.0.1:5141) 1，(listen *:5142)和(listen 5143)为 0

    lsopt.wildcard = u.wildcard;
#if (NGX_HAVE_INET6 && defined IPV6_V6ONLY)
    lsopt.ipv6only = 1;
#endif
    // TIP: 将 lsopt.u.sockaddr 中数值形式的 port 和 addr，转成 127.0.0.1:5141/0.0.0.0:5142 字符串的形式保存到 lsopt.addr
    (void)ngx_sock_ntop(&lsopt.u.sockaddr, lsopt.socklen, lsopt.addr,
                        NGX_SOCKADDR_STRLEN, 1);
...
ngx_http_core_server_name(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    for (i = 1; i < cf->args->nelts; i++)
    {
        ...
        { // 如果是$hostname，则直接存储gethostname系统调用得到的主机名
...
{ //(error_pages内容是从ngx_http_error_pages中取的)
    ...
    if (value[i].data[0] == '=')
    {
        ...
        //=后面如果跟新的返回码，则必须紧跟=后面
        if (value[i].len > 1)
        { // error_page 404 =200 /empty.gif;，表示以200作为新的返回码，用户可以通过“=”来更改返回的错误码
            // error_page 404 =200 /empty.gif;
        ...
        { // error_page 404 = /empty.gif; 表示返回码由重定向后实际处理的真实结果来决定，这时，只要把“=”后面没有返回码号
            overwrite = 0;
        }
    ...
    { // error_page错误码必须must be between 300 and 599，并且不能为499
