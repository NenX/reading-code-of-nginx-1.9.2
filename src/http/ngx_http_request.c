...

{
    ...
    rev = c->read;
    rev->handler = ngx_http_wait_request_handler; // 1.0.15 为 ngx_http_init_request
    ...
    if (rev->ready)
    {
        ...
        rev->handler(rev); // ngx_http_wait_request_handler 1.0.15 为 ngx_http_init_request
        return;
    }
    ...
    */
    ...
    // TIP: 3. ngx_http_init_connection -> ngx_handle_read_event -> ngx_add_event -> ngx_epoll_add_event

    if (ngx_handle_read_event(rev, 0, NGX_FUNC_LINE) != NGX_OK)
    { // 当下次有数据从客户端发送过来的时候，会在ngx_epoll_process_events把对应的ready置1。
        ngx_http_close_connection(c);
        return;
    }
}
...
ngx_http_wait_request_handler(ngx_event_t *rev)
{
    ...
    // TIP: rev->handler = ngx_http_process_request_line;
...
ngx_http_process_request_line(ngx_event_t *rev) // ngx_http_process_request_line方法来接收HTTP请求行
{
    ...
    for (;;)
    {
        ...
        { // 请求行解析成功
            ...
            { // 1.0以下版本没有请求头部字段，
                /*
                    ...
                    */
                if (r->headers_in.server.len == 0 && ngx_http_set_virtual_server(r, &r->headers_in.server) // http0.9应该是从请求行获取虚拟主机?
                                                         == NGX_ERROR)
                {
                    return;
                }
            ...
            // TIP: rev->handler = ngx_http_process_request_headers;
        ...
        if (rc != NGX_AGAIN)
        { // 读取完毕内核该套接字上面的数据，头部行不全，则说明头部行不全关闭连接
...
ngx_http_process_request_headers(ngx_event_t *rev)
{
    ...
    for (;;)
    {
        ...
        if (rc == NGX_AGAIN)
        {
            ...
            if (r->header_in->pos == r->header_in->end)
            { // 说明header_in空间已经用完了，无法继续存储recv的数据，则重新分配大空间
...
ngx_http_read_request_header(ngx_http_request_t *r)
{
    ...
    if (n == NGX_AGAIN)
    { // 如果recv返回NGX_AGAIN,则需要重新add read event,这样下次有数据来可以继续读,
...
*/
// ngx_http_process_request_headers头部行解析完毕后调用函数ngx_http_process_request_header
// TIP: a0 ngx_http_process_request: 设置 c->write->handler = ngx_http_request_handler，然后调用 ngx_http_handler, 最后调用 ngx_http_run_posted_requests
void ngx_http_process_request(ngx_http_request_t *r)
{
     ...
     */
    if (c->read->timer_set)
    { // ngx_http_read_request_header中读取不到数据的时候返回NGX_AGIN，会添加定时器和读事件表示继续等待客户端数据到来
...
ngx_http_set_virtual_server(ngx_http_request_t *r, ngx_str_t *host)
{
    ...
    {
        if (hc->ssl_servername->len == host->len && ngx_strncmp(hc->ssl_servername->data,
                                                                host->data, host->len) == 0)
        {
#if (NGX_PCRE)
            if (hc->ssl_servername_regex && ngx_http_regex_exec(r, hc->ssl_servername_regex,
...
// TIP: a2 ngx_http_request_handler: 调用 request 的 write_event_handler 或者 read_event_handler, 最后调用 ngx_http_run_posted_requests
static void
ngx_http_request_handler(ngx_event_t *ev)
{
    ...
    */
    ...
    if (ev->write)
    {                              // 说明ev是ngx_connection_t->write
        r->write_event_handler(r); // ngx_http_core_run_phases
    }
    else
    { // 说明ev是ngx_connection_t->read事件
...
{                                                 // subrequest注意ngx_http_run_posted_requests与ngx_http_postpone_filter ngx_http_finalize_request配合阅读
    ...
    */
    for (;;)
    { /* 遍历该客户端请求r所对应的所有subrequest，然后把这些subrequest请求做重定向处理 */
...
可以继续调用NGX_HTTP_CONTENT_PHASE阶段的其他处理方法。
    ...
    { // 例如还有未发送的数据，见ngx_http_copy_filter，则buffered不为0
...
ngx_http_set_write_handler(ngx_http_request_t *r)
{
    ...
    clcf = ngx_http_get_module_loc_conf(r, ngx_http_core_module);
    if (!wev->delayed)
    { // 本次ngx_http_write_filter没有把数据发送完毕，则需要添加写事件定时器
        /*
...
ngx_http_lingering_close_handler(ngx_event_t *rev)
{
    ...
    if (rev->timedout)
    { // 该定时器在ngx_http_set_lingering_close设置的
        ngx_http_close_request(r, 0);
        return;
    }
...
*/
void ngx_http_free_request(ngx_http_request_t *r, ngx_int_t rc) // 释放request的相关资源
{
    ngx_log_t *log;
    ngx_pool_t *pool;
    ...
    { // 减少time_wait状态，内核的四次挥手,加快四次挥手。只有在关闭套接字的时候内核还有数据没有接收或者发送才会发送RST
        clcf = ngx_http_get_module_loc_conf(r, ngx_http_core_module);
