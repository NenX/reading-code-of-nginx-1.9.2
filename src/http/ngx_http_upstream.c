...
// 在读取完浏览器发送来的请求头部字段后，会通过proxy fastcgi等模块读取包体，读取完后执行该函数，例如ngx_http_read_client_request_body(r, ngx_http_upstream_init);
{
    ...
    { // 如果epoll使用边缘触发
        ...
        */
        ...
        {
            //???: 为什么走到这里面
            // 要增加可写事件通知，为啥?因为待会可能就能写了,可能会转发上游服务器的内容给浏览器等客户端
            // 实际上是检查和客户端的连接是否异常了
            char tmpbuf[256];
            ...
            //TIP:监听当前fd的写事件，在下面的ngx_http_upstream_init_request中设置 read_event_handler 和 write_event_handler
...
ngx_http_upstream_init_request(ngx_http_request_t *r)
{
    ...
    if (r->request_body)
    {                                            // 客户端发送过来的POST数据存放在此,ngx_http_read_client_request_body放的
    ...
    { // ngx_http_XXX_create_request   ngx_http_proxy_create_request等
        ngx_http_finalize_request(r, NGX_HTTP_INTERNAL_SERVER_ERROR);
        return;
    }
    ...
    { // 数组upstream_states，保留upstream的状态信息。
    ...
    */
    if (u->resolved == NULL)
    {                             // 上游的IP地址是否被解析过，ngx_http_fastcgi_handler调用ngx_http_fastcgi_eval会解析。 为NULL说明没有解析过，也就是fastcgi_pas xxx中的xxx参数没有变量
        uscf = u->conf->upstream; // upstream赋值在ngx_http_fastcgi_pass
    }
    else
    { // fastcgi_pass xxx的xxx中有变量，说明后端服务器是会根据请求动态变化的，参考ngx_http_fastcgi_handler
        ...
        { // 如果地址已经被resolve过了，我IP地址，此时创建round robin peer就行
        ...
        { // 遍历所有的上游模块，根据其host进行查找，找到host,port相同的。

            uscf = uscfp[i]; // 找一个IP一样的上流模块

            if (uscf->host.len == host->len && ((uscf->port == 0 && u->resolved->no_port) || uscf->port == u->resolved->port) && ngx_strncasecmp(uscf->host.data, host->data, host->len) == 0)
            {
...
found:
    ...
    { // ngx_http_upstream_init_round_XX_peer(ngx_http_upstream_init_round_robin_peer)
        ngx_http_upstream_finalize_request(r, u,
                                           NGX_HTTP_INTERNAL_SERVER_ERROR);
        return;
    }
...
ngx_http_upstream_cache(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    { /* 如果还未给当前请求分配缓存相关结构体( ngx_http_cache_t ) 时，创建此类型字段( r->cache ) 并初始化： */
        ...
        { ////解析xx_cache_key adfaxx 参数值到r->cache->keys
            return NGX_ERROR;
        }
                            ...
                            ngx_http_file_cache_t **cache)
{
    ...
    { // 在u->caches中查找proxy_cache或者fastcgi_cache xxx$ss解析出的xxx$ss字符串，是否有相同的
                               ...
                               ngx_uint_t do_write) // 向上游服务器发送请求   当一次发送不完，通过ngx_http_upstream_send_request_handler再次触发发送
{
    ...
    if (rc == NGX_AGAIN)
    { // 协议栈缓冲区已满，需要等待发送数据出去后出发epoll可写，从而继续write
        if (!c->write->ready)
        {
        ...
        { // 例如ngx_http_upstream_send_request_body发送了三次返回NGX_AGAIN,那么第二次就需要把第一次上面的超时定时器关了，表示发送正常
    ...
    if (c->write->timer_set)
    { // 这里的定时器是ngx_http_upstream_connect中connect返回NGX_AGAIN的时候添加的定时器
        /*
2025/04/24 02:54:29[             ngx_event_connect_peer,    32]  [debug] 14867#14867: *1 socket 12
2025/04/24 02:54:29[           ngx_epoll_add_connection,  1486]  [debug] 14867#14867: *1 epoll add connection: fd:12 ev:80002005
2025/04/24 02:54:29[             ngx_event_connect_peer,   125]  [debug] 14867#14867: *1 connect to 127.0.0.1:3666, fd:12 #2
                                    ...
                                    ngx_http_upstream_t *u, ngx_uint_t do_write)
{
    ...
    if (!u->request_sent)
    {
        ...
        {
            for (cl = out; cl->next; cl = out->next)
            { /* void */
            }
                                       ...
                                       ngx_http_upstream_t *u)
{
    ...
    // 表示向上游服务器发送的请求已经超时
    if (c->write->timedout)
    { // 该定时器在ngx_http_upstream_send_request添加的
    ...
    if (u->header_sent)
    { // 都已经收到后端的数据并且发送给客户端浏览器了，说明不会再想后端写数据，
...
{ // 读取FCGI头部数据，或者proxy头部数据。ngx_http_upstream_send_request发送完数据后，
    ...
    if (c->read->timedout)
    { // 读超时了，轮询下一个。 ngx_event_expire_timers超时后走到这里
    ...
    { // 分配一块缓存，用来存放接受回来的数据。
        ...
        <title>Your page Subject and domain name</title>
          */
        if (r->cache)
        { // 注意这里跳过了预留的头部内存，用于存储cache写入文件时候的头部部分，见
            u->buffer.pos += r->cache->header_start;
            u->buffer.last = u->buffer.pos;
        }
#endif
    ...
    for (;;)
    {
        ...
        if (rc == NGX_AGAIN)
        {
            ...
            { // 分配的用来存储fastcgi STDOUT头部行包体的buf已经用完了头部行都还没有解析完成，
                ngx_log_error(NGX_LOG_ERR, c->log, 0,
...
ngx_http_upstream_process_headers(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    for (i = 0; /* void */; i++)
    {
        ...
        if (hh)
        { // 一个个拷贝到请求的headers_out里面
            if (hh->copy_handler(r, &h[i], hh->conf) != NGX_OK)
            { // 从u->headers_in.headers复制到r->headers_out.headers用于发送
                ngx_http_upstream_finalize_request(r, u,
                                                   NGX_HTTP_INTERNAL_SERVER_ERROR);
                return NGX_DONE;
            }
                                         ...
                                         ngx_http_upstream_t *u)
{
    ...
    if (rev->timedout)
    { // 在发送请求到后端的时候，我们需要等待对方应答，因此设置了读超时定时器，见ngx_http_upstream_send_request
...
ngx_http_upstream_send_response(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    if (r->header_only)
    { // 如果只需要发送头部数据，比如客户端用curl -I 访问的。返回204状态码即可。

        if (!u->buffering)
        { // 配置不需要缓存包体，或者后端要求不配置缓存包体，直接结束
            ngx_http_upstream_finalize_request(r, u, rc);
            return;
        }

        if (!u->cacheable && !u->store)
        { // 如果定义了#if (NGX_HTTP_CACHE)则可能置1
            ngx_http_upstream_finalize_request(r, u, rc);
            return;
        }
    ...
    { // 客户端发送过来的包体存储在临时文件中，则需要把存储临时文件删除
    ...
    if (!u->buffering)
    {
        ...
        if (u->input_filter == NULL)
        { // 如果input_filter为空，则设置默认的filter，然后准备发送数据到客户端。然后试着读读FCGI
            u->input_filter_init = ngx_http_upstream_non_buffered_filter_init;
        ...
        if (n)
        { // 得到将要发送的数据的大小，每次有多少就发送多少。不等待upstream了  因为这是不缓存方式发送包体到客户端
            u->buffer.last = u->buffer.pos;
    ...
    if (u->cacheable)
    {
        ...
        if (valid == 0)
        { // 赋值proxy_cache_valid xxx 4m;中的4m
            valid = ngx_http_file_cache_valid(u->conf->cache_valid,
                                              u->headers_in.status_n);
            if (valid)
            {
    ...
    if (r->cache && r->cache->file_cache->temp_path && r->cache->file_cache->path)
    {
        ngx_log_debugall(p->log, 0, "ngx_http_upstream_send_response, "
                                    "p->cacheable:%i, tempfile:%V, pathfile:%V",
                         cacheable, &r->cache->file_cache->temp_path->name, &r->cache->file_cache->path->name);
    }
...
*/
    ...
    if (u->cacheable)
    { // 注意走到这里的时候，前面已经把后端头部行信息解析出来了，u->buffer.pos指向的是实际数据部分
                                   ...
                                   ngx_uint_t from_upstream, ngx_uint_t do_write)
{
    ...
    if ((upstream->read->eof && u->buffer.pos == u->buffer.last) || (downstream->read->eof && u->from_client.pos == u->from_client.last) || (downstream->read->eof && upstream->read->eof))
    {
        ngx_log_debug0(NGX_LOG_DEBUG_HTTP, c->log, 0,
    ...
    if (ngx_handle_write_event(upstream->write, u->conf->send_lowat, NGX_FUNC_LINE) != NGX_OK)
    {
        ngx_http_upstream_finalize_request(r, u, NGX_ERROR);
        return;
    }
    ...
    if (ngx_handle_write_event(downstream->write, clcf->send_lowat, NGX_FUNC_LINE) != NGX_OK)
    {
        ngx_http_upstream_finalize_request(r, u, NGX_ERROR);
        return;
    }
                                               ...
                                               ngx_uint_t do_write)
{
    ...
    for (;;)
    {
        ...
        { // 要立即发送。
            ...
            if (u->busy_bufs == NULL)
            { // busy_bufs没有了，都发完了。想要发送的数据都已经发送完毕

                if (u->length == 0 || (upstream->read->eof && u->length == -1)) // 包体数据已经读完了
                {
                    ngx_http_upstream_finalize_request(r, u, 0);
                    return;
                }
        ...
        if (size && upstream->read->ready)
        {
            ...
            if (n == NGX_AGAIN)
            { // 说明已经内核缓冲区数据已经读完，退出循环，然后根据epoll事件来继续触发读取后端数据
                break;
            }
    ...
    if (ngx_handle_read_event(upstream->read, 0, NGX_FUNC_LINE) != NGX_OK)
    { // epoll在accept的时候读写已经加入epoll中，因此对epoll来说没用
        ngx_http_upstream_finalize_request(r, u, NGX_ERROR);
        return;
    }
...
ngx_http_upstream_non_buffered_filter(void *data, ssize_t bytes)
{
    ...
    *ll = cl; // 将新申请的缓存链接进来。
    ...
    if (u->length == -1)
    { // u->length表示将要发送的数据大小如果为-1,则说明后端协议并没有指定需要发送的大小(例如chunk方式)，此时我们只需要发送我们接收到的.
        return NGX_OK;
    }
...
{ // 注意走到这里的时候，后端发送的头部行信息已经在前面的ngx_http_upstream_send_response->ngx_http_send_header已经把头部行部分发送给客户端了
    ...
    if (u->peer.connection)
    {
        ...
        if (u->store)
        {
            ...
            { // 本次内核缓冲区数据读取完毕，或者后端所有数据读取完毕

                tf = p->temp_file;

                if (u->headers_in.status_n == NGX_HTTP_OK && (p->upstream_done || p->length == -1) && (u->headers_in.content_length_n == -1 || u->headers_in.content_length_n == tf->offset))
                {
...
ngx_http_upstream_store(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    if (tf->file.fd == NGX_INVALID_FILE)
    {
        ...
        if (ngx_create_temp_file(&tf->file, tf->path, tf->pool,
                                 tf->persistent, tf->clean, tf->access) != NGX_OK)
        {
            return;
        }
                       ...
                       ngx_uint_t ft_type) // 和后端某个服务器交互出错(例如connect)，则选择下一个后端服务器，同时标记该服务器出错
{
    ...
    if (u->peer.sockaddr)
    {
        ...
        if (ft_type == NGX_HTTP_UPSTREAM_FT_HTTP_403 || ft_type == NGX_HTTP_UPSTREAM_FT_HTTP_404) // 后端服务器拒绝服务，表示还是可用的，只是拒绝了当前请求
        {
            state = NGX_PEER_NEXT;
        }
        else
    ...
    if (status)
    {
        ...
        if (u->peer.tries == 0 || !(u->conf->next_upstream & ft_type) || (u->request_sent && r->request_body_no_buffering) || (timeout && ngx_current_msec - u->peer.start_time >= timeout)) // 判断是否需要重新连接下一个后端服务器，不需要则直接返回错误给客户端
        {
#if (NGX_HTTP_CACHE)
                                   ...
                                   ngx_http_upstream_t *u, ngx_int_t rc)
{
    ...
    *u->cleanup = NULL;
    ...
    if (u->peer.connection)
    { // 如果是设置了keepalive num配置，则在ngx_http_upstream_free_keepalive_peer中会把u->peer.connection置为NULL,避免关闭连接，缓存起来避免重复建立和关闭连接
    ...
    if (r->header_only)
    { // 只发送头部行，请求后端的头部行在ngx_http_upstream_send_response->ngx_http_send_header已经发送
        ngx_http_finalize_request(r, rc);
        return;
    }
                                      ...
                                      ngx_uint_t offset)
{
    ...
    if (*ph == NULL)
    { // 这个就是坚持r->upstream->headers_in中的offset成员处是否有存放该ngx_table_elt_t的空间
        *ph = h;
    }
                                        ...
                                        ngx_table_elt_t *h, ngx_uint_t offset)
{
    ...
    *ph = h;
...
#if (NGX_HTTP_CACHE)
    {
        ...
        if (ngx_strlcasestrn(start, last, (u_char *)"no-cache", 8 - 1) != NULL || ngx_strlcasestrn(start, last, (u_char *)"no-store", 8 - 1) != NULL || ngx_strlcasestrn(start, last, (u_char *)"private", 7 - 1) != NULL)
        {
            u->cacheable = 0;
            return NGX_OK;
        }
                                      ...
                                      ngx_http_variable_value_t *v, uintptr_t data)
{
    ...
    if (r->upstream == NULL || !r->upstream->conf->cache_revalidate || r->upstream->cache_status != NGX_HTTP_CACHE_EXPIRED || r->cache->last_modified == -1)
    {
        v->not_found = 1;
        return NGX_OK;
    }
...
ngx_http_upstream_cache_etag(ngx_http_request_t *r,
                             ngx_http_variable_value_t *v, uintptr_t data)
{
    if (r->upstream == NULL || !r->upstream->conf->cache_revalidate || r->upstream->cache_status != NGX_HTTP_CACHE_EXPIRED || r->cache->etag.len == 0)
    {
        v->not_found = 1;
        return NGX_OK;
    }
...
{ // 当碰到upstream{}指令的时候调用这里。
    ...
    uscf = ngx_http_upstream_add(cf, &u, NGX_HTTP_UPSTREAM_CREATE | NGX_HTTP_UPSTREAM_WEIGHT | NGX_HTTP_UPSTREAM_MAX_FAILS | NGX_HTTP_UPSTREAM_FAIL_TIMEOUT | NGX_HTTP_UPSTREAM_DOWN | NGX_HTTP_UPSTREAM_BACKUP);
    if (uscf == NULL)
    {
                                    ...
                                    ngx_str_t *default_hide_headers, ngx_hash_init_t *hash)
{
    ...
    { // 把default_hide_headers中的元素赋值给hide_headers数组中
    ...
    if (conf->hide_headers != NGX_CONF_UNSET_PTR)
    { // proxy_hide_header  fastcgi_hide_header配置的相关信息也要添加到hide_headers数组
    ...
    if (conf->pass_headers != NGX_CONF_UNSET_PTR)
    { // proxy_pass_headers  fastcgi_pass_headers配置的相关信息从hide_headers数组
...
ngx_http_upstream_create_main_conf(ngx_conf_t *cf)
{
    ...
    if (ngx_array_init(&umcf->upstreams, cf->pool, 4,
                       sizeof(ngx_http_upstream_srv_conf_t *)) != NGX_OK)
    {
        return NULL;
    }
