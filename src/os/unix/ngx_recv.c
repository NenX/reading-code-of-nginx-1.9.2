...
ngx_unix_recv(ngx_connection_t *c, u_char *buf, size_t size)
{
    ...
    do
    {
        ...
        if (n == 0)
        {                   // 表示TCP错误，见ngx_http_read_request_header   recv返回0表示对方已经关闭连接 The return value will be 0 when the peer has performed an orderly shutdown.
        ...
        else if (n > 0)
        {
            ...
            if ((size_t)n < size && !(ngx_event_flags & NGX_USE_GREEDY_EVENT)) // 数据读取完毕ready置0,需要重新添加add epoll event
            {
